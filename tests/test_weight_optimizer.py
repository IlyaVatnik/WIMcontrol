...
def test_optimizer(W_true,xl_true,xr_true,calibration_file_path,
                   noise_level):
        ...
        for ch in channels:
            for fbg in fbgs_per_channel[ch]:
                ...
                predicted = (W / 2.0) * (response_function(xl, A, mu, sigma) +
                                         response_function(xl+wheelset_width, A, mu, sigma))
                # Измеренный сдвиг
    ...
    measured_shifts=get_meas_shifts(W_true, xl_true, xr_true)
    history = []
    # Начальное приближение
    x_0=0
    wheelset_width=80
    x0=[200,x_0,wheelset_width]
...
if __name__=='__main__':
    ...
    W_opt, xl_opt, wheelset_width_opt, xr_opt,iterations,history=test_optimizer(W_true,xl_true,xr_true,calibration_file_path,noise_level)
